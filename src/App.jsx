...
// RUT formatter: input "123456789" → "12.345.678-9"
const formatRut = (raw) => {
...
const Stat = ({ label, value, sub, color }) => (
  <div style={{ padding:"20px 24px", background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, flex:1, minWidth:140 }}>
    <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:6 }}>{label}</div>
    <div style={{ fontFamily:FONT_DISPLAY, fontSize:26, fontWeight:700, color:color||COLORS.text }}>{value}</div>
    {sub && <div style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted, marginTop:4 }}>{sub}</div>}
  </div>
);
...
const Modal = ({ title, onClose, onSubmit, children }) => (
  <div style={{ position:"fixed", inset:0, background:"#000A", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center" }}>
    <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:32, width:480, maxHeight:"80vh", overflowY:"auto" }}>
  ...
  return (
    <div>
      <div style={{ marginBottom:28 }}>
        ...
        <div style={{ fontFamily:FONT_DISPLAY, fontSize:26, fontWeight:700, color:COLORS.text }}>Dashboard B2B</div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:20, marginBottom:24 }}>
        <div style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:24 }}>
          <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, color:COLORS.text, marginBottom:18, fontSize:15 }}>Embudo de ventas</div>
          {stageData.map(s=>(
            <div key={s.key} style={{ marginBottom:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                <span style={{ fontFamily:FONT, fontSize:12, color:s.color }}>{s.label}</span>
                <span style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted }}>{s.count} · {fmt(s.value)}</span>
              </div>
              <div style={{ height:6, background:COLORS.border, borderRadius:3 }}>
                <div style={{ height:6, borderRadius:3, background:s.color, width:`${(s.value/maxVal)*100}%` }} />
              </div>
            </div>
          ))}
        </div>
        <div style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:24 }}>
          <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, color:COLORS.text, marginBottom:18, fontSize:15 }}>Próximas tareas</div>
          ...
          {recentTasks.map(t=>(
            <div key={t.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:`1px solid ${COLORS.border}` }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.text }}>{t.title}</div>
                <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted }}>{t.company}</div>
              </div>
      ...
      <div style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:24 }}>
        <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, color:COLORS.text, marginBottom:18, fontSize:15 }}>Deals activos — mayor valor</div>
        {deals.filter(d=>d.stage!=="cerrado").length===0 && <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.textMuted }}>Sin deals activos aún.</div>}
            ...
            {deals.filter(d=>d.stage!=="cerrado").sort((a,b)=>Number(b.value)-Number(a.value)).slice(0,5).map(d=>{
              ...
              return (
                <tr key={d.id} style={{ borderBottom:`1px solid ${COLORS.border}` }}>
                  <td style={{ fontFamily:FONT, fontSize:13, color:COLORS.text, padding:"12px 0" }}>{d.title}</td>
                  <td style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted, padding:"12px 16px 12px 0" }}>{d.company}</td>
                  <td style={{ padding:"12px 16px 12px 0" }}><Badge color={stage.color}>{stage.label}</Badge></td>
                  <td style={{ fontFamily:FONT, fontSize:13, color:COLORS.accent }}>{fmt(d.value)}</td>
                  <td style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted }}>{fmtDate(d.closeDate)}</td>
                </tr>
              );
            })}
          </tbody>
...
function ContactsView({ contacts, setContacts }) {
  ...
  const openNew = () => {
    ...
    setForm({ name:"", company:"", role:"", email:"", phone:"", rut:"", status:"lead", value:"", lastContact:"" });
    setShowModal(true);
  };

  const openEdit = (c) => {
    setEditingContact(c.id);
    setForm({ name:c.name, company:c.company, role:c.role||"", email:c.email||"", phone:c.phone||"", rut:c.rut||"", status:c.status, value:String(c.value||0), lastContact:c.lastContact||"" });
    setShowModal(true);
  };
  ...
  const save = () => {
    ...
    let updated;
    if (editingContact) {
      updated = contacts.map(c => c.id===editingContact ? { ...c, ...form, value:Number(form.value)||0 } : c);
      if (selected?.id===editingContact) setSelected({ ...selected, ...form, value:Number(form.value)||0 });
    } else {
      updated = [...contacts, { ...form, id:uid(), value:Number(form.value)||0, lastContact:form.lastContact||new Date().toISOString().slice(0,10) }];
    }
    setContacts(updated); saveData("crm_contacts", updated);
    setShowModal(false);
    setEditingContact(null);
    setForm({ name:"", company:"", role:"", email:"", phone:"", rut:"", status:"lead", value:"", lastContact:"" });
  };
  ...
  const del = (id) => {
    ...
    setContacts(updated); saveData("crm_contacts", updated); setSelected(null);
  };
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <div>
          ...
          <div style={{ fontFamily:FONT_DISPLAY, fontSize:26, fontWeight:700, color:COLORS.text }}>Contactos B2B</div>
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <div style={{ position:"relative" }}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por nombre, empresa o RUT…"
              style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:6, padding:"8px 14px 8px 36px", fontFamily:FONT, fontSize:13, color:COLORS.text, outline:"none", width:280 }} />
            <span style={{ position:"absolute", left:11, top:"50%", transform:"translateY(-50%)", fontSize:14, color:COLORS.textMuted }}>🔍</span>
            {search && <button onClick={()=>setSearch("")} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:COLORS.textMuted, cursor:"pointer", fontSize:14, lineHeight:1 }}>✕</button>}
          </div>
          {["todos","cliente","prospecto","lead"].map(s=>(
            <button key={s} onClick={()=>setFilterStatus(s)} style={{ padding:"8px 14px", borderRadius:6, fontFamily:FONT, fontSize:12, cursor:"pointer", background:filterStatus===s?COLORS.accent:COLORS.card, color:filterStatus===s?COLORS.bg:COLORS.textMuted, border:`1px solid ${filterStatus===s?COLORS.accent:COLORS.border}` }}>{s.charAt(0).toUpperCase()+s.slice(1)}</button>
          ))}
      ...
      {search && (
        <div style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted, marginBottom:16 }}>
          {filtered.length} resultado{filtered.length!==1?"s":""} para <span style={{ color:COLORS.accent }}>"{search}"</span>
        </div>
      )}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(300px,1fr))", gap:16 }}>
        {filtered.map(c=>{
          ...
          return (
            <div key={c.id} onClick={()=>setSelected(c)} style={{ background:COLORS.card, border:`1px solid ${selected?.id===c.id?COLORS.accent:COLORS.border}`, borderRadius:10, padding:20, cursor:"pointer", transition:"all 0.2s" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, fontSize:15, color:COLORS.text }}>{c.name}</div>
                  ...
                  {c.rut && <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textDim, marginTop:2 }}>RUT: {c.rut}</div>}
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  <button onClick={e=>{e.stopPropagation(); openEdit(c);}} style={{ background:"none", border:`1px solid ${COLORS.accent}44`, borderRadius:4, color:COLORS.accent, cursor:"pointer", fontSize:12, padding:"2px 7px" }}>✏️</button>
                  <Badge color={sc.color}>{sc.label}</Badge>
                </div>
              </div>
              <div style={{ borderTop:`1px solid ${COLORS.border}`, paddingTop:12, display:"flex", justifyContent:"space-between" }}>
                <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted }}>{c.email}</div>
                <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.green, fontWeight:600 }}>{fmt(c.value)}</div>
              </div>
            </div>
          );
        })}
      ...
      {selected && (
        <div style={{ position:"fixed", top:0, right:0, width:360, height:"100%", background:COLORS.surface, borderLeft:`1px solid ${COLORS.border}`, padding:32, overflowY:"auto", zIndex:100 }}>
          <button onClick={()=>setSelected(null)} style={{ background:"none", border:"none", color:COLORS.textMuted, cursor:"pointer", fontFamily:FONT, fontSize:12, marginBottom:24 }}>← Cerrar</button>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:4 }}>
            <div style={{ fontFamily:FONT_DISPLAY, fontSize:22, fontWeight:700, color:COLORS.text }}>{selected.name}</div>
          ...
          <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.accent, marginBottom:16 }}>{selected.role} — {selected.company}</div>
          ...
          <div style={{ marginTop:24, display:"flex", flexDirection:"column", gap:12 }}>
            {[["RUT",selected.rut||"—"],["Email",selected.email],["Teléfono",selected.phone],["Último contacto",fmtDate(selected.lastContact)],["Valor total",fmt(selected.value)]].map(([k,v])=>(
              <div key={k} style={{ background:COLORS.card, borderRadius:8, padding:"12px 16px", border:`1px solid ${COLORS.border}` }}>
                <div style={{ fontFamily:FONT, fontSize:10, color:COLORS.textMuted, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:4 }}>{k}</div>
                <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.text }}>{v||"—"}</div>
              </div>
            ))}
          </div>
          <button onClick={()=>del(selected.id)} style={{ marginTop:24, width:"100%", padding:"10px 0", background:"transparent", border:`1px solid ${COLORS.red}44`, borderRadius:6, color:COLORS.red, fontFamily:FONT, fontSize:12, cursor:"pointer" }}>Eliminar contacto</button>
        </div>
      )}

      {showModal && (
        <Modal title={editingContact ? "Editar Contacto" : "Nuevo Contacto"} onClose={()=>setShowModal(false)} onSubmit={save}>
          ...
          <Input label="Cargo" value={form.role} onChange={e=>f("role",e.target.value)} placeholder="Ej: CTO" />
...
function PipelineView({ deals, setDeals, contacts }) {
  ...
  const openNew = () => {
    setEditingDeal(null);
    setForm({ title:"", company:"", contactId:"", rut:"", value:"", stage:"contacto", probability:"20", closeDate:"" });
    setShowModal(true);
  };

  const openEdit = (d) => {
    setEditingDeal(d.id);
    setForm({ title:d.title, company:d.company, contactId:d.contactId||"", rut:d.rut||"", value:String(d.value), stage:d.stage, probability:String(d.probability), closeDate:d.closeDate||"" });
    setShowModal(true);
  };
  ...
  const save = () => {
    ...
    let updated;
    if (editingDeal) {
      updated = deals.map(d => d.id===editingDeal ? { ...d, ...form, value:Number(form.value)||0, probability:Number(form.probability)||20 } : d);
    } else {
      updated = [...deals, { ...form, id:uid(), value:Number(form.value)||0, probability:Number(form.probability)||20 }];
    }
    setDeals(updated); saveData("crm_deals", updated);
  ...
  const toggleAll = () => {
    const next = {};
    deals.forEach(d => { next[d.id] = !allCollapsed; });
    setCollapsed(next);
  };
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:24 }}>
        <div>
          ...
          <div style={{ fontFamily:FONT_DISPLAY, fontSize:26, fontWeight:700, color:COLORS.text }}>Pipeline de Ventas</div>
        </div>
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={toggleAll} style={{ padding:"9px 16px", background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:7, color:COLORS.textMuted, fontFamily:FONT_DISPLAY, fontSize:12, cursor:"pointer" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16 }}>
        {STAGES.map(stage=>{
          ...
          return (
            <div key={stage.key}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, padding:"10px 14px", background:COLORS.card, borderRadius:8, border:`1px solid ${stage.color}33` }}>
                <div>
                ...
                <div style={{ width:26, height:26, borderRadius:"50%", background:stage.color+"22", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:FONT, fontSize:12, color:stage.color, fontWeight:700 }}>{stageDeals.length}</div>
              </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {stageDeals.map(d=>{
                  ...
                  return (
                    <div key={d.id} style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:8, borderLeft:`3px solid ${stage.color}`, overflow:"hidden", transition:"all 0.2s" }}>

                      {/* ── HEADER (always visible) ── */}
                      <div style={{ display:"flex", alignItems:"center", gap:8, padding: isCollapsed ? "10px 12px" : "12px 16px 8px" }}>
                        <button onClick={()=>toggleCollapse(d.id)} style={{ background:"none", border:"none", color:COLORS.textMuted, cursor:"pointer", fontSize:12, padding:0, flexShrink:0, lineHeight:1 }}>
                          {isCollapsed ? "▶" : "▼"}
                        </button>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontFamily:FONT_DISPLAY, fontSize:13, fontWeight:600, color:COLORS.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{d.title}</div>
                          {isCollapsed && <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted }}>{d.company}</div>}
                        </div>
                        {isCollapsed && (
                          <div style={{ fontFamily:FONT, fontSize:12, color:COLORS.green, fontWeight:700, flexShrink:0 }}>{fmt(d.value)}</div>
                        )}
                      ...
                      {!isCollapsed && (
                        <div style={{ padding:"0 16px 14px" }}>
                          <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted, marginBottom:d.rut?2:10 }}>{d.company}</div>
                          {d.rut && <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textDim, marginBottom:10 }}>RUT: {d.rut}</div>}
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                            <div style={{ fontFamily:FONT, fontSize:14, color:COLORS.green, fontWeight:700 }}>{fmt(d.value)}</div>
                            <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted }}>{fmtDate(d.closeDate)}</div>
                          </div>
                          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                            <span style={{ fontFamily:FONT, fontSize:10, color:COLORS.textMuted }}>Probabilidad</span>
                            <span style={{ fontFamily:FONT, fontSize:10, color:stage.color }}>{d.probability}%</span>
                          </div>
                          <div style={{ height:4, background:COLORS.border, borderRadius:2, marginBottom:12 }}>
                            <div style={{ height:4, borderRadius:2, background:stage.color, width:`${d.probability}%` }} />
                          </div>
                          <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                            {STAGES.filter(s=>s.key!==stage.key).map(s=>(
                              <button key={s.key} onClick={()=>moveDeal(d.id,s.key)} style={{ padding:"3px 8px", borderRadius:4, fontFamily:FONT, fontSize:10, cursor:"pointer", background:"transparent", border:`1px solid ${s.color}44`, color:s.color }}>→ {s.label}</button>
                            ))}
                            <button onClick={()=>del(d.id)} style={{ padding:"3px 8px", borderRadius:4, fontFamily:FONT, fontSize:10, cursor:"pointer", background:"transparent", border:`1px solid ${COLORS.red}44`, color:COLORS.red, marginLeft:"auto" }}>✕</button>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {stageDeals.length===0 && <div style={{ border:`1px dashed ${COLORS.border}`, borderRadius:8, padding:"24px 0", textAlign:"center", fontFamily:FONT, fontSize:12, color:COLORS.textDim }}>Sin deals</div>}
              </div>
            </div>
          );
        })}
      ...
      {showModal && (
        <Modal title={editingDeal ? "Editar Deal" : "Nuevo Deal"} onClose={()=>setShowModal(false)} onSubmit={save}>
          <Input label="Título *" value={form.title} onChange={e=>f("title",e.target.value)} placeholder="Ej: Licencia Enterprise" />
          <Input label="Empresa *" value={form.company} onChange={e=>f("company",e.target.value)} placeholder="Ej: Nexum Corp" />
          ...
          <Input label="Valor" value={form.value} onChange={e=>f("value",e.target.value)} placeholder="0" type="number" />
...
function TasksView({ tasks, setTasks, contacts, isMobile }) {
  ...
  const toggle = (id) => {
    ...
    setTasks(updated); saveData("crm_tasks", updated);
  };
  const del = (id) => {
  ...
  const save = () => {
    ...
    const updated = [...tasks, { ...form, id:uid(), done:false, company:form.company||(contact?.company||"") }];
    setTasks(updated); saveData("crm_tasks", updated);
    setShowModal(false);
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <div>
          ...
          <div style={{ fontFamily:FONT_DISPLAY, fontSize:26, fontWeight:700, color:COLORS.text }}>Tareas</div>
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          {["todas","pendientes","vencidas","completadas"].map(flt=>(
            <button key={flt} onClick={()=>setFilter(flt)} style={{ padding:"8px 14px", borderRadius:6, fontFamily:FONT, fontSize:12, cursor:"pointer", background:filter===flt?COLORS.accent:COLORS.card, color:filter===flt?COLORS.bg:COLORS.textMuted, border:`1px solid ${filter===flt?COLORS.accent:COLORS.border}` }}>{flt.charAt(0).toUpperCase()+flt.slice(1)}</button>
          ))}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {filtered.map(t=>{
          ...
          return (
            <div key={t.id} style={{ background:COLORS.card, border:`1px solid ${overdue?COLORS.red+"44":COLORS.border}`, borderRadius:8, padding:"14px 20px", display:"flex", alignItems:"center", gap:16, opacity:t.done?0.5:1 }}>
              <div onClick={()=>toggle(t.id)} style={{ width:20, height:20, borderRadius:4, cursor:"pointer", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", background:t.done?COLORS.green:"transparent", border:`2px solid ${t.done?COLORS.green:COLORS.border}` }}>
                {t.done && <span style={{ color:COLORS.bg, fontSize:12, fontWeight:700 }}>✓</span>}
              </div>
              <div style={{ fontSize:18 }}>{TYPE_ICONS[t.type]||"✅"}</div>
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:FONT_DISPLAY, fontSize:14, color:COLORS.text, textDecoration:t.done?"line-through":"none" }}>{t.title}</div>
                <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted, marginTop:2 }}>{t.company}</div>
              </div>
              <Tag label={pc.label} color={pc.color} />
              <div style={{ fontFamily:FONT, fontSize:12, color:overdue?COLORS.red:COLORS.textMuted, minWidth:60, textAlign:"right" }}>{overdue&&"⚠ "}{fmtDate(t.dueDate)}</div>
              <button onClick={()=>del(t.id)} style={{ background:"none", border:"none", color:COLORS.textDim, cursor:"pointer", fontSize:14 }}>✕</button>
            </div>
          );
        })}
      ...
      {showModal && (
        <Modal title="Nueva Tarea" onClose={()=>setShowModal(false)} onSubmit={save}>
          ...
          <Input label="Empresa" value={form.company} onChange={e=>f("company",e.target.value)} placeholder="Ej: DataFlow SA" />
  ...
  return (
    <div>
      <div style={{ marginBottom:28 }}>
        ...
        <div style={{ fontFamily:FONT_DISPLAY, fontSize:26, fontWeight:700, color:COLORS.text }}>Reportes y Estadísticas</div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:20, marginBottom:20 }}>
        <div style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:24 }}>
          <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, color:COLORS.text, marginBottom:18, fontSize:15 }}>Pipeline por etapa</div>
          {STAGES.map(s=>{
            ...
            return (
              <div key={s.key} style={{ marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                  <span style={{ fontFamily:FONT, fontSize:12, color:s.color }}>{s.label}</span>
                  <span style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted }}>{deals.filter(d=>d.stage===s.key).length} deals · {fmt(val)}</span>
                </div>
                <div style={{ height:8, background:COLORS.border, borderRadius:4 }}>
                  <div style={{ height:8, borderRadius:4, background:s.color, width:`${totalPipeline>0?(val/totalPipeline)*100:0}%` }} />
                </div>
              </div>
            );
          })}
        ...
        <div style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:24 }}>
          <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, color:COLORS.text, marginBottom:18, fontSize:15 }}>Contactos por estado</div>
          {Object.entries(STATUS_CONFIG).map(([key,sc])=>{
            ...
            return (
              <div key={key} style={{ marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                  <span style={{ fontFamily:FONT, fontSize:12, color:sc.color }}>{sc.label}</span>
                  <span style={{ fontFamily:FONT, fontSize:12, color:COLORS.textMuted }}>{count} ({pct}%)</span>
                </div>
                <div style={{ height:8, background:COLORS.border, borderRadius:4 }}>
                  <div style={{ height:8, borderRadius:4, background:sc.color, width:`${pct}%` }} />
                </div>
              </div>
            );
          })}
      ...
      <div style={{ background:COLORS.card, border:`1px solid ${COLORS.border}`, borderRadius:10, padding:24 }}>
        <div style={{ fontFamily:FONT_DISPLAY, fontWeight:600, color:COLORS.text, marginBottom:18, fontSize:15 }}>Top empresas por valor</div>
        {contacts.length===0 && <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.textMuted }}>Sin datos aún. Agrega contactos.</div>}
        {[...contacts].sort((a,b)=>Number(b.value)-Number(a.value)).slice(0,6).map((c,i)=>(
          <div key={c.id} style={{ display:"flex", alignItems:"center", gap:14, padding:"10px 0", borderBottom:`1px solid ${COLORS.border}` }}>
            <div style={{ fontFamily:FONT, fontSize:13, color:COLORS.textDim, width:20 }}>#{i+1}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:FONT_DISPLAY, fontSize:13, color:COLORS.text }}>{c.company}</div>
              <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.textMuted }}>{c.name}</div>
            </div>
...
export default function CRM() {
  ...
  useEffect(()=>{
    ...
    setContacts(c); setDeals(d); setTasks(t); setLoading(false);
  },[]);
  ...
  if(loading) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:COLORS.bg }}>
      <div style={{ fontFamily:FONT, color:COLORS.accent, fontSize:14, letterSpacing:"0.1em" }}>Cargando NexusCRM…</div>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", flexDirection: isMobile ? "column" : "row", minHeight:"100vh", background:COLORS.bg, fontFamily:FONT_DISPLAY }}>
      ...
      {/* ── MOBILE HEADER ── */}
      {isMobile && (
        <header style={{ background:COLORS.surface, borderBottom:`1px solid ${COLORS.border}`, padding:"14px 20px", display:"flex", justifyContent:"space-between", alignItems:"center", position:"sticky", top:0, zIndex:150 }}>
          <div>
            <div style={{ fontFamily:FONT, fontSize:9, color:COLORS.accent, letterSpacing:"0.18em", textTransform:"uppercase" }}>B2B SALES</div>
            <div style={{ fontFamily:FONT_DISPLAY, fontSize:16, fontWeight:700, color:COLORS.text }}>Polygonos SpA</div>
          </div>
          <button onClick={()=>setMenuOpen(p=>!p)} style={{ background:"none", border:`1px solid ${COLORS.border}`, borderRadius:7, color:COLORS.text, cursor:"pointer", padding:"8px 12px", fontSize:18 }}>
            {menuOpen ? "✕" : "☰"}
          </button>
        </header>
      )}
      ...
      {isMobile && menuOpen && (
        <div style={{ position:"fixed", top:62, left:0, right:0, background:COLORS.surface, borderBottom:`1px solid ${COLORS.border}`, zIndex:140, padding:"12px" }}>
          {NAV.map(n=>{
            const active=view===n.key;
            return (
              <button key={n.key} onClick={()=>navigate(n.key)} style={{ display:"flex", alignItems:"center", gap:12, width:"100%", padding:"14px 16px", borderRadius:8, marginBottom:4, background:active?COLORS.accentDim:"transparent", border:`1px solid ${active?COLORS.accentGlow:"transparent"}`, cursor:"pointer", color:active?COLORS.accent:COLORS.text, fontFamily:FONT_DISPLAY, fontSize:15, fontWeight:active?600:400, textAlign:"left" }}>
      ...
      {!isMobile && (
        <aside style={{ width:220, background:COLORS.surface, borderRight:`1px solid ${COLORS.border}`, padding:"28px 0", display:"flex", flexDirection:"column", flexShrink:0 }}>
          ...
          <div style={{ padding:"20px 24px", borderTop:`1px solid ${COLORS.border}` }}>
            ...
            <div style={{ fontFamily:FONT_DISPLAY, fontSize:13, color:COLORS.text, fontWeight:600 }}>Tu CRM</div>
            <div style={{ fontFamily:FONT, fontSize:11, color:COLORS.green }}>Datos guardados ✓</div>
          </div>
        </aside>
      )}
          ...
          {NAV.map(n=>{
            ...
            return (
              <button key={n.key} onClick={()=>navigate(n.key)} style={{ flex:1, padding:"10px 4px 8px", background:"transparent", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
                ...
                <span style={{ fontFamily:FONT, fontSize:9, color:active?COLORS.accent:COLORS.textMuted, letterSpacing:"0.05em" }}>{n.label}</span>
